...
import { verifyApproveToken, escapeHtml } from '@/lib/digest';
import { sendNitrosendCampaign } from '@/lib/nitrosend';
...
export const GET: APIRoute = async ({ url }) => {
  ...
  if (!nitroKey) return html(500, '<h2>Server misconfigured</h2><p>NITROSEND_API_KEY not set.</p>');
  ...
  try {
    await sendNitrosendCampaign(nitroKey, r.broadcast_id);
  } catch (err) {
  ...
  return html(200, '<h2 style="color:#0f766e;">Digest sent</h2><p>The campaign has been queued in NitroSend.</p>');
};
